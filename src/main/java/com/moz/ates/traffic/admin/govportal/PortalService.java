...
public class PortalService {
    ...
    public void registNotice(MozBrd brd, MultipartFile[] uploadFiles) {
    	...
    	for(MultipartFile uploadFile : uploadFiles) {
    		if(!uploadFile.isEmpty() && !MozatesCommonUtils.isNull(uploadFile.getOriginalFilename())){
    			String[] extArr = {"doc", "docx", "xls", "xlsx", "pdf", "zip"};
    ...
    public void deleteNotice(String boardIdx) {
    	...
    	List<MozAtchFile> atchFileList = mozAtchFileRepository.findAllMozAtchFileByAtchIdxAndRgsTy(boardIdx, RegistantType.ADMIN_USER);
    	if(!atchFileList.isEmpty()) {
    		for (MozAtchFile atchFile : atchFileList) {
     ...
     * @return : 
     */
	public List<MozAtchFile> findAllMozAtchFileByAtchIdx(String inqryId, String rgsTy) {
		return mozAtchFileRepository.findAllMozAtchFileByAtchIdxAndRgsTy(inqryId, rgsTy);
	}
	...
	/**
     * @brief : unnecessaryQna
     * @details : 스팸처리
     * @author : KY.LEE
     * @date : 2024.05.23
     * @param : inqryId
     */
	public void unnecessaryQna(MozInqry mozInqry) {
		mozInqry.setNcsYn("N");
		mozInqryRepository.updateNcsYn(mozInqry);
	}
}
