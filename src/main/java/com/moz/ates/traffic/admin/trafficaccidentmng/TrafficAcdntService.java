...
public class TrafficAcdntService {
    	...
    	if(!acdntTrgtInfoList.isEmpty()) {
    		for(MozTfcAcdntTrgtInfo trgtInfo : acdntTrgtInfoList) {
    			...
    			if(!MozatesCommonUtils.isNull(trgtInfo.getAcdntTrgtCd()) 
    				...
    				if(acdntTrgtPnrInfoList != null && !acdntTrgtPnrInfoList.isEmpty()) {
    ...
    public MozTfcAcdntMaster getMngDetail(String tfcAcdntId) {
    	...
    	// 타겟 및 탑승자 정보 조회
		List<MozTfcAcdntTrgtInfo> targetList = tfcAcdntTrgtInfoRepository.findAllByTfcAdcntId(tfcAcdntId);

		for (MozTfcAcdntTrgtInfo target : targetList) {
			target.setTfcAcdntTrgtPnrInfo(tfcAcdntTrgtPnrInfoRepository.findAllByTfcAcdntTrgtId(target.getAcdntTrgtId()));
		}

    	tfcAcdntMaster.setTfcAcdntTrgtInfo(targetList);
    ...
    public void upateAcdnt(MozTfcAcdntMaster tfcAcdntMaster, MultipartFile[] uploadFiles) {
		...
		for(MozTfcAcdntTrgtInfo dbTrgtInfo : dbTrgtInfoList) {
			...
			if(acdntTrgtInfoList != null && !acdntTrgtInfoList.isEmpty()) {
				for(MozTfcAcdntTrgtInfo trgtInfo : acdntTrgtInfoList) {
	 ...
	 */
	public List<MozTfcAcdntTrgtInfo> getAcdntTrgtListByDvrLcenId(MozTfcAcdntTrgtInfo tfcAcdntTrgtInfo) {
		return tfcAcdntTrgtInfoRepository.findAllTfcAcdntTrgtByDvrLcenId(tfcAcdntTrgtInfo);
	}

	/**
	 * @brief 사고타겟 이력 카운트 조회
	 * @details : 사고타겟 이력 카운트 조회
	 * @author IK.MOON
	 * @date 2024. 8. 20.
	 * @method getAcdntTrgtCountByDvrLcenId
	 */
	public int getAcdntTrgtCountByDvrLcenId(MozTfcAcdntTrgtInfo tfcAcdntTrgtInfo) {
		return tfcAcdntTrgtInfoRepository.countAllTfcAcdntTrgtByDvrLcenId(tfcAcdntTrgtInfo);
	}
	 ...
	 */
	public List<MozTfcAcdntTrgtInfo> getAcdntTrgtListByDocNidOrvhRegNo(MozTfcAcdntTrgtInfo tfcAcdntTrgtInfo) {
		return tfcAcdntTrgtInfoRepository.findAllTfcAcdntTrgtByDocNidOrVhRegNo(tfcAcdntTrgtInfo);
	}
	 ...
	 * @method getAcdntTrgtCountByDvrLcenId
	 */
	public int getAcdntTrgtCountByDocNidOrVhRegNo(MozTfcAcdntTrgtInfo tfcAcdntTrgtInfo) {
		return tfcAcdntTrgtInfoRepository.countAllTfcAcdntTrgtByDocNidOrVhRegNo(tfcAcdntTrgtInfo);
	}
}
